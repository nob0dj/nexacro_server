 ...
 */
@WebServlet("/empl/search")
public class EmplSearchServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EmplSearchServlet() {
        super();
...
//		System.out.println(request.getMethod());//POST
		...
		//HttpPlatformRequest - PlatformData
		...
		//사용자 입력값 처리 : 
		//하나의 입력값만 있는 경우, VariableList를 사용하지 않고, getVariable(name)으로 가져올 수 있다.
		Variable var = reqPlatformData.getVariable("search");
		String search = var.getString();
		...
		try {
			...
			List<Employee> list = employeeService.searchEmpl(search);
